...
var HEAD=JSON.stringify({"Authorization":access_token});
    ...
    JSON.stringify([{name:"原始文件",url:JSON.parse(xincode).items[0].download_url,head:{"Authorization":access_token,"Referer":"https://www.aliyundrive.com/"}},{name:"LQ低音质转码",url:转码HQ,head:{"Referer":"https://www.aliyundrive.com/"}},{name:"本地LQ低音质转码",url:本地转码HQ,head:{"Referer":"https://www.aliyundrive.com/"}}]);
        ...
        var downloadurl='http://103.106.189.99:3000/apis/my-yun-play/'+file_id+'/'+drive_id+'/'+access_token+'/alidownloadurl/'+mydrive_id+'/'+user_id+'/'+device_id+"/"+videoname;
        var 新原画code=getHttp(downloadurl)
        var 播放模式=getVar("播放模式")||"全部";
            if(播放模式=="全部"||播放模式=="null"){
                JSON.stringify([{name:"新原画",url:JSON.parse(新原画code).url,head:{"Authorization":access_token,"Referer":"https://www.aliyundrive.com/"}},{name:"原画",url:JSON.parse(xincode).items[0].download_url,head:{"Authorization":access_token,"Referer":"https://www.aliyundrive.com/"}},{name:"本地高画质转码",url:本地高画质转码,head:{"Referer":"https://www.aliyundrive.com/"}},{name:"720P转码",url:转码720,head:{"Referer":"https://www.aliyundrive.com/"}},{name:"高画质转码",url:高画质转码,head:{"Referer":"https://www.aliyundrive.com/"}}]);
            }else if(播放模式=="本地转码"){
                JSON.stringify([{name:"新原画",url:JSON.parse(新原画code).url,head:{"Authorization":access_token,"Referer":"https://www.aliyundrive.com/"}},{name:"原画",url:JSON.parse(xincode).items[0].download_url,head:{"Authorization":access_token,"Referer":"https://www.aliyundrive.com/"}},{name:"本地高画质转码",url:本地高画质转码,head:{"Referer":"https://www.aliyundrive.com/"}}]);
            }else{
