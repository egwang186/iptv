...
},
{
    "title":"阿里小纸条",
    "rule":{
        "分类":'var a=JSON.parse(getHttp(JSON.stringify({url:"https://gitcafe.net/alipaper/home.json",head:{"Referer":"https://u.gitcafe.net/"}}))).data;var xindata=[];for(var i in a){xindata.push({title:a[i].name+a[i].cat_num,url:"https://gitcafe.net/tool/alipaper/$$action=viewcat&num=#PN#&cat="+a[i].code})}JSON.stringify(xindata);',
