...
"翻页"+b;
##免嗅探规则
var onedata=getHttp(getVar("地址").split("?wd=")[1]);if(onedata.search(/sources:.+?file:".+?"/)!=-1){var u=onedata.match(/sources:.+?file:"(.+?)"/)[1];}else if(onedata.search(/src:\s*?'http.+?'/)!=-1){var u=onedata.match(/src:\s*?'(http.+?)'/)[1];}else if(onedata.search(/var urlPlay = 'http.+?'/)!=-1){var u=onedata.match(/var urlPlay = '(http.+?)'/)[1];}else{var script=onedata.match(/<script.+?>eval([\s\S]+?)<\/script>/)[1];var dedata=eval(script);var u=dedata.match(/sources:.+?file:"(.+?)"/)[1];}var 头=u.match(/http.+\//)[0];var code=getHttp(u).match(/#EXT-X-STREAM-INF[^#]+/g);var playlist=[];for(var i in code){var title=code[i].match(/RESOLUTION=(.+)/)[1];var url=头+code[i].replace(/\s+/g,'"').match(/.+"(.+?)"/)[1];playlist.push({name:title,url:url});}JSON.stringify(playlist);
####
