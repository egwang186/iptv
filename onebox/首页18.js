...
"翻页"+b;
##免嗅探规则
var onedata=getHttp(getVar("地址").split("?wd=")[1]);if(onedata.search(/sources:.+?file:.*?".+?"/)!=-1){var u=onedata.match(/sources:.+?file:.*?"(.+?)"/);}else if(onedata.search(/src:.*?'http.+?'/)!=-1){var u=onedata.match(/src:.*?'(http.+?)'/);}else if(onedata.search(/var urlPlay = 'http.+?'/)!=-1){var u=onedata.match(/var urlPlay = '(http.+?)'/);}else if(nedata.search(/<script.+?>eval[\s\S]+?<\/script>/)!=-1){var script=onedata.match(/<script.+?>eval([\s\S]+?)<\/script>/)[1];var dedata=eval(script);var u=dedata.match(/sources:.+?file:"(.+?)"/);}else{var u="";}if(u){var u=u[1];var 头=u.match(/http.+\//)[0];var code=getHttp(JSON.stringify({url:u,head:{"Referer":getVar("地址").split("?wd=")[1]}})).match(/#EXT-X-STREAM-INF[^#]+/g);var playlist=[];for(var i in code){var title=code[i].match(/RESOLUTION=([^,'"]+)/)[1];var url=头+code[i].replace(/\s+/g,'"').match(/.+"(.+?)"/)[1];playlist.push({name:title,url:url,head:{"Referer":getVar("地址").split("?wd=")[1]}});}JSON.stringify(playlist);}else{JSON.stringify([{name:"地址获取错误",url:"",head:{"Referer":getVar("地址").split("?wd=")[1]}}]);}
####
