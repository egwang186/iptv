...
"翻页"+b;
##免嗅探规则
var onedata=getHttp(getVar("地址").split("?wd=")[1]);var source=e2(onedata,".get(video source).a(src)");if(source.indexOf("http")!=-1){var du=source;}else if(onedata.search(/sources:.+?file:.*?".+?"/)!=-1){var u=onedata.match(/sources:.+?file:.*?"(.+?)"/);}else if(onedata.search(/src:.*?'http.+?'/)!=-1){var u=onedata.match(/src:.*?'(http.+?)'/);}else if(onedata.search(/var urlPlay = 'http.+?'/)!=-1){var u=onedata.match(/var urlPlay = '(http.+?)'/);}else if(onedata.search(/<script.+?>eval[\s\S]+?<\/script>/)!=-1){var script=onedata.match(/[\S\s]+<script.+?>eval([\s\S]+?)<\/script>/)[1];var dedata=eval(script);var u=dedata.match(/sources:.+?file:"(.+?)"/);}else{var uu=onedata.match(/'(\/pass_md5.+?)'/);if(uu){var uu=getVar("地址").split("?wd=")[1].match(/(http.+?\/\/.+?)\//)[1]+uu[1];var du=getHttp(JSON.stringify({url:uu,head:{"User-Agent":"Mozilla/5.0 Android","Referer":getVar("地址").split("?wd=")[1]}})).match(/http.+/)[0];}else{var u="";}}if(u){var u=u[1];var 头=u.match(/http.+\//)[0];var code=getHttp(JSON.stringify({url:u,head:{"User-Agent":"Mozilla/5.0 Android"}})).match(/#EXT-X-STREAM-INF[^#]+/g);var playlist=[];for(var i in code){var title=code[i].match(/RESOLUTION=([^,'"]+)/)[1];var url=code[i].replace(/\s+/g,'"').match(/.+"(.+?)"/)[1];if(url.indexOf("http")==-1){url=头+url;}playlist.push({name:title,url:url,head:{"Referer":getVar("地址").split("?wd=")[1]}});}JSON.stringify(playlist);}else{if(du){JSON.stringify([{name:"地址带Referer",url:du,head:{"User-Agent":"Mozilla/5.0 Android","Referer":getVar("地址").split("?wd=")[1]}}]);}else{JSON.stringify([{name:"地址获取错误",url:"",head:{"Referer":getVar("地址").split("?wd=")[1]}}]);}}
####
