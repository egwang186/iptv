...
##搜索规则
var 源码=getVar("源码");var 列表=e2Arr(源码,".get(div.video_list_1)");if(!列表[0]){var 列表=["网页可能有搜索验证，去网页看看吧"];var 标题规则=".t()";var 地址规则=getVar("baseURL");var 图片规则=".get(img).a(data-original)";var 简介规则="长按网页打开，验证后返回重新搜索即可";var 图片底部规则="";var 左上规则="";var 右上规则="";var NEXTPAGE="";var PREPAGE="";}else{var 标题规则=".get(div.video_list_1_2).t()";var 地址规则=".get(a).a(href)";var 图片规则=".get(img).a(src)";var 简介规则=".get(.img_hengtiao_1).t().c(|).get(.img_hengtiao_2).t()";var 图片底部规则="";var 左上规则="";var 右上规则=".z(vip)";var NEXTPAGE="";var PREPAGE="";}
##搜索翻页
var b="";for(var i=1;i<50;i=i+1){b=b+"+第"+i+"页="+i;}
"翻页"+b;
##免嗅探规则
...
##选集规则
var 分类=e2Arr(getVar("源码"),".all()");var 简介=getVar("标题");var 线路="";var 列表规则=".json(zhubo)";var 标题规则=getVar("标题");var 选集规则=".json(title)";var 选集地址规则="@js=\"http://ip111.cn/?wd=\"+e2(列表[j],\".json(address)\").replace(/\\?/g,\"幻魔快修\");";
##搜索规则
...
##搜索规则
var 源码=getVar("源码");var 列表=e2Arr(源码,".get(ul.videos li)");if(!列表[0]){var 列表=["网页可能有搜索验证，去网页看看吧"];var 标题规则=".t()";var 地址规则=getVar("baseURL");var 图片规则=".get(img).a(data-original)";var 简介规则="长按网页打开，验证后返回重新搜索即可";var 图片底部规则="";var 左上规则="";var 右上规则="";var NEXTPAGE="";var PREPAGE="";}else{var 标题规则=".get(img).a(alt)";var 地址规则=".get(a).a(href)";var 图片规则=".get(img).a(src)";var 简介规则=".get(span.video-overlay).t().c().get(span.left).t().c().get(span.right).t()";var 图片底部规则="";var 左上规则="";var 右上规则="";var NEXTPAGE="";var PREPAGE="";}
##搜索翻页
var b="";for(var i=1;i<50;i=i+1){b=b+"+第"+i+"页="+i;}
"翻页"+b;
##免嗅探规则
var onedata=getHttp(getVar("地址").split("?wd=")[1]);var source=e2(onedata,".get(video source).a(src)");if(source.indexOf("http")!=-1){var du=source;}else if(source.indexOf("//")==0){var du="https:"+source;}else if(onedata.search(/sources:.+?file:.*?".+?"/)!=-1){var u=onedata.match(/sources:.+?file:.*?"(.+?)"/);}else if(onedata.search(/src:.*?'http.+?'/)!=-1){var u=onedata.match(/src:.*?'(http.+?)'/);}else if(onedata.search(/var urlPlay = 'http.+?'/)!=-1){var u=onedata.match(/var urlPlay = '(http.+?)'/);}else if(onedata.search(/<script.+?>eval[\s\S]+?<\/script>/)!=-1){var script=onedata.match(/[\S\s]+<script.+?>eval([\s\S]+?)<\/script>/)[1];var dedata=eval(script);var u=dedata.match(/sources:.+?file:"(.+?)"/);}else{var uu=onedata.match(/'(\/pass_md5.+?)'/);if(uu){var uu=getVar("地址").split("?wd=")[1].match(/(http.+?\/\/.+?)\//)[1]+uu[1];var du=getHttp(JSON.stringify({url:uu,head:{"User-Agent":"Mozilla/5.0 Android","Referer":getVar("地址").split("?wd=")[1]}})).match(/http.+/)[0];function makePlay(){for(var a="",t="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",n=t.length,o=0;10>o;o++)a+=t.charAt(Math.floor(Math.random()*n));return a+"?token=hkx7y4fo0zsggcbyvn7v0kn0&expiry="+Date.now();}du=du+makePlay();}else{var u="";}}if(u){var u=u[1];var 头=u.match(/http.+\//)[0];var code=getHttp(JSON.stringify({url:u,head:{"User-Agent":"Mozilla/5.0 Android"}})).match(/#EXT-X-STREAM-INF[^#]+/g);var playlist=[];for(var i in code){var title=code[i].match(/RESOLUTION=([^,'"]+)/)[1];var url=code[i].replace(/\s+/g,'"').match(/.+"(.+?)"/)[1];if(url.indexOf("http")==-1){url=头+url;}playlist.push({name:title,url:url,head:{"Referer":getVar("地址").split("?wd=")[1]}});}JSON.stringify(playlist);}else{if(du){JSON.stringify([{name:"地址带Referer",url:du,head:{"User-Agent":"Mozilla/5.0 Android","Referer":getVar("地址").split("?wd=")[1]}}]);}else{JSON.stringify([{name:"地址获取错误",url:"",head:{"Referer":getVar("地址").split("?wd=")[1]}}]);}}
####
...
##分类规则
ar 列表=e2Arr(getVar("源码"),".z(\\{\"id\":.+?\\})");var 标题规则=".json(name)";var 地址规则=".c(/zh/video/).json(id).ct(.json)";var 图片规则="@js=var id=JSON.parse(CODE).id;var 前=id.split('-')[0];var 后=id.split('-')[1];if(id.indexOf('FC2-')==0){'https://ccavb.tv/static/img/'+前+'/'+后.substr(0,2)+'/'+后+'/'+id+'_1.jpg'}else{'https://ccavb.tv/static/img/'+前+'/'+后+'/'+id+'_1.jpg'}";var 简介规则=".json(types)";var 图片底部规则=".json(updateTime)";var 左上规则="";var 右上规则="";var NEXTPAGE="";var PREPAGE="";
##选集规则
